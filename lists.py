...
def long_words(words):
    ...
    four_chars = [word for word in words if len(word) > 4]
    return four_chars
...
def halvesies(numbers):
    ...
    half_nums = [x/2.0 for x in numbers]
...
def mult_numbers(numbers):
    ...
    total_mult = reduce((lambda x, y: x * y), numbers)
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    count = 0
    index = len(items) - 2
    """Pop at index, add to the end the index - 1 numbers of times"""
...
def duplicates(items):
    ...
    doubles = set([x for x in items if items.count(x) > 1])
